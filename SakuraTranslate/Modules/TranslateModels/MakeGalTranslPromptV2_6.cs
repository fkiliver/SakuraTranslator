...
namespace SakuraTranslate
{
    ...
    {
        private string MakeGalTranslPromptV2_6(string originalText, double frequencyPenalty = 0)
        {
            var dictStr = GetDictStr(originalText);
            return MakeRequestStr(new List<PromptMessage>
            {
                ...
                new PromptMessage
                {
                    ...
                    Content = $"参考以下术语表（可为空，格式为src->dst #备注）：{(string.IsNullOrEmpty(dictStr) ? string.Empty : "\n" + dictStr)}\n\n" +
                              $"根据上述术语表的对应关系和备注，结合历史剧情和上下文，以流畅的风格将下面的文本从日文翻译成简体中文：{originalText}"
                }
            }, GetMaxTokens(originalText), 0.2, 0.8, frequencyPenalty);
        }
    }
}